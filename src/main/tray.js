...
const t = {
  ...
  saveLayout:           isJa ? '今の配置を保存'                    : 'Save Current Layout',
  ...
  noWindowsDetail:      isJa ? 'アクセシビリティ権限が付与されているか確認してください。' : 'Make sure accessibility permissions are granted.',
  savedMsg:             (name) => isJa ? `「${name}」を保存しました` : `"${name}" saved`,
  savedDetail:          (n) => isJa ? `${n} 個のウィンドウを記録しました。` : `${n} window${n === 1 ? '' : 's'} captured.`,
  deleteConfirm:        (name) => isJa ? `「${name}」を削除しますか？` : `Delete "${name}"?`,
  deleteBtn:            isJa ? '削除'                              : 'Delete',
  cancelBtn:            isJa ? 'キャンセル'                        : 'Cancel',
  permOkMsg:            isJa ? 'アクセシビリティ権限が有効です'     : 'Accessibility permission is enabled.',
  ok:                   'OK',
}
...
class TrayManager {
  ...
  rebuildMenu() {
    ...
    const layoutItems = layouts.length > 0
      ? layouts.map((layout) => ({
          label: layout.name,
          submenu: [
            {
