...
function buildRestoreScript(appName, appWindows) {
  ...
  const windowBlocks = appWindows.map((win, listIdx) => {
    const escapedTitle = (win.windowTitle || '').replace(/\\/g, '\\\\').replace(/"/g, '\\"')
    return `
  ...
  const sortedByIndex = [...appWindows]
    .sort((a, b) => (a.windowIndex || 1) - (b.windowIndex || 1))
  const zOrderBlocks = sortedByIndex.map((win) => {
    const escapedTitle = (win.windowTitle || '').replace(/\\/g, '\\\\').replace(/"/g, '\\"')
    return `
  repeat with w in wins
    if name of w is "${escapedTitle}" then
...
async function captureCurrentWindows() {
  try {
    ...
    return result.split(', ').map((line) => {
      ...
      const [appName, bundleId, windowTitle, xStr, yStr, wStr, hStr, idxStr] = parts
...
async function restoreLayout(windows) {
  ...
  for (const win of windows) {
    const key = win.bundleId && win.bundleId !== 'missing value' ? win.bundleId : win.appName
